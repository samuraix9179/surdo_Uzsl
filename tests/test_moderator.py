"""Test moderator video quality operations for the Surdo project."""
import sys
import os
...
def test_analyze_video_quality_sync_valid(mock_exists):
    """Verify that video analyzer returns valid true and 1.0 probability when hands and face are present."""
...
def test_analyze_video_quality_sync_invalid_face(mock_exists):
    """Verify that video analyzer returns valid false and lower probability when face is missing."""
    # Setup cv2.VideoCapture mock
    cap_inst = mock_cv2.VideoCapture.return_value
