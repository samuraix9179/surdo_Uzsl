"""Tests for validating agent_tasks.json schema and data."""
import pytest
...
@pytest.fixture
def valid_manifest():
    """Return a valid mock manifest for testing purposes."""
    return {
        "schema_version": 1,
        "project": "surdo",
...
def test_missing_tasks(valid_manifest):
    """Verify validation fails if tasks list is missing."""
...
def test_empty_tasks(valid_manifest):
    """Verify validation fails if tasks list is empty."""
...
def test_invalid_task_status(valid_manifest):
    """Verify validation fails if a task has an invalid status."""
...
def test_duplicate_task_id(valid_manifest):
    """Verify validation fails if multiple tasks share the same ID."""
