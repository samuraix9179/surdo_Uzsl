"""Test cases for validating the agent tasks manifest."""
import pytest
...
@pytest.fixture
def valid_manifest():
    """Return a baseline valid task manifest dictionary."""
    return {
        "schema_version": 1,
        "project": "surdo",
...
def test_valid_manifest(valid_manifest):
    """Verify that a correctly formatted manifest passes validation."""
    warnings = validate_manifest(valid_manifest)
    assert isinstance(warnings, list)


def test_missing_schema_version(valid_manifest):
    """Verify validation fails if the schema version is missing."""
...
def test_invalid_schema_version(valid_manifest):
    """Verify validation fails if the schema version is unsupported."""
...
def test_missing_tasks(valid_manifest):
    """Verify validation fails if the tasks list is completely missing."""
...
def test_empty_tasks(valid_manifest):
    """Verify validation fails if the tasks list is empty."""
...
def test_invalid_task_status(valid_manifest):
    """Verify validation fails if a task has an invalid status string."""
...
def test_missing_task_fields(valid_manifest):
    """Verify validation fails if required fields are missing from a task."""
...
def test_duplicate_task_id(valid_manifest):
    """Verify validation fails if duplicate task IDs exist."""
