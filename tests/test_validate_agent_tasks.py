"""Test the validation of agent tasks manifest."""
import pytest
...
@pytest.fixture
def valid_manifest():
    """Fixture to provide a valid manifest for testing."""
    return {
        "schema_version": 1,
        "project": "surdo",
...
def test_valid_manifest(valid_manifest):
    """Test that a valid manifest does not raise any errors."""
    warnings = validate_manifest(valid_manifest)
    assert isinstance(warnings, list)


def test_missing_schema_version(valid_manifest):
    """Test that validation fails if schema_version is missing."""
...
def test_invalid_schema_version(valid_manifest):
    """Test that validation fails if schema_version is invalid."""
...
def test_missing_tasks(valid_manifest):
    """Test that validation fails if tasks list is missing."""
...
def test_empty_tasks(valid_manifest):
    """Test that validation fails if tasks list is empty."""
...
def test_invalid_task_status(valid_manifest):
    """Test that validation fails if task status is invalid."""
...
def test_missing_task_fields(valid_manifest):
    """Test that validation fails if required task fields are missing."""
...
def test_duplicate_task_id(valid_manifest):
    """Test that validation fails if duplicate task IDs exist."""
