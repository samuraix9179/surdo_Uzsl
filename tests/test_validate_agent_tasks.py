"""Test module for verifying the agent_tasks.json validation script."""
...
@pytest.fixture
def valid_manifest():
    """Provide a valid mock manifest for testing purposes."""
    return {
        "schema_version": 1,
        "project": "surdo",
...
def test_valid_manifest(valid_manifest):
    """Verify that a properly formatted manifest passes validation without errors."""
    warnings = validate_manifest(valid_manifest)
    assert isinstance(warnings, list)


def test_missing_schema_version(valid_manifest):
    """Verify that validation fails if schema_version is missing."""
...
def test_invalid_schema_version(valid_manifest):
    """Verify that validation fails if schema_version is not 1."""
...
def test_missing_tasks(valid_manifest):
    """Verify that validation fails if the tasks array is missing."""
...
def test_empty_tasks(valid_manifest):
    """Verify that validation fails if the tasks array is empty."""
...
def test_invalid_task_status(valid_manifest):
    """Verify that validation fails if a task has an unrecognized status."""
...
def test_missing_task_fields(valid_manifest):
    """Verify that validation fails if a task is missing required fields."""
...
def test_duplicate_task_id(valid_manifest):
    """Verify that validation fails if multiple tasks share the same ID."""
