"""Training module for UZSL isolated sign language recognition.

Handles training models such as SSTCN and SLGCN on isolated signs.
"""
...
class UZSLDataset(Dataset):  # type: ignore[type-arg]
    ...
    def __init__(self, data_dir: str = LANDMARKS_DIR, target_frames: int = NUM_FRAMES) -> None:
        """Initialize the UZSLDataset.

        Args:
            data_dir (str): Path to directory with landmark data. Defaults to LANDMARKS_DIR.
            target_frames (int): Number of frames to interpolate sequences to. Defaults to NUM_FRAMES.
        """
...
def train() -> None:
    """Train the isolated sign language recognition model."""
