...
class UZSLContinuousDataset(Dataset):  # type: ignore[type-arg]
    ...
    def __init__(self, data_dir: str = LANDMARKS_DIR, target_frames: int = NUM_FRAMES) -> None:
        """Initialize continuous SLR dataset."""
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, int, int]:
        """Get item from continuous dataset."""
        return (
            self.samples[idx],
            self.targets[idx],
...
def collate_fn(
    ...
    """Apply custom collate function to handle variable length targets for CTC Loss."""
