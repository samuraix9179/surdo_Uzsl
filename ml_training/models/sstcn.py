"""Module containing the SSTCN (Separable Spatial-Temporal Convolution Network) model."""
...
class SeparableSTBlock(nn.Module):  # type: ignore[misc]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass for the SeparableSTBlock.
...
class SSTCN(nn.Module):  # type: ignore[misc]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass for the SSTCN model.

        Args:
            x: Input tensor of shape (batch_size, in_channels, num_frames, num_nodes).

        Returns:
            Output tensor of shape (batch_size, num_classes) probabilities.
        """
        out = self.proj(x)
        out = self.block1(out)
        out = self.block2(out)
