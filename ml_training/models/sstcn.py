"""SSTCN model architecture for SLR."""
import torch
...
class SeparableSTBlock(nn.Module):  # type: ignore[misc]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor): Input shape: (batch_size, in_channels, num_frames, num_nodes).
...
class SSTCN(nn.Module):  # type: ignore[misc]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Logits (batch_size, num_classes) probabilities.
        """
        out = self.proj(x)
        out = self.block1(out)
        out = self.block2(out)
