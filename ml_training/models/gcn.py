"""Graph Convolution Network (GCN) primitives module.

Contains graph and temporal convolution layers used in SLGCN.
"""
...
class GraphConvolution(nn.Module):  # type: ignore[misc]
    ...
    Applies spatial graph convolution based on the adjacency matrix:
...
class TemporalConvolution(nn.Module):  # type: ignore[misc]
    ...
    Attributes:
        conv (nn.Conv2d): 2D Convolutional layer acting as 1D temporal convolution.
    ...
    def __init__(
        ...
        Args:
            in_channels (int): Number of input channels.
            out_channels (int): Number of output channels.
            kernel_size (int, optional): Kernel size for the temporal convolution. Defaults to 9.
